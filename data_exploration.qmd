...
\``{r} detail <- airbnb_clean |>   filter(!(grepl("Madrid|马德里|馬德里自治區", City, ignore.case = TRUE) | grepl("28...", Zipcode)))`
...
####Neighbourhood There seems to be a difference between Neighbourhood and Neighbouhood.Cleansed. Let's visualize that:
...
write.csv(airbnb_clean, "airbnb_clean.csv", sep = ";", row.names = FALSE)
```
