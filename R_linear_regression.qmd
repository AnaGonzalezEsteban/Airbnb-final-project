...
There are some serious outliers in this column, but given the nature of the dataset a lot of these can be real observations. Let's have a look at the percentiles.

```{r}
quantile(df_airbnb$Price,c(0, 0.25, 0.5, 0.75, 0.9, 0.95, 0.98, 0.995, 1))

```

It should be enough to just trim the last 0.5 percentile instead of applying the iqr\*1,5 approach.

```{r}
df_airbnb |>
  mutate(Price=ifelse(Price>410, 410, Price)) -> df_short
```

```{r}
ggplot(df_short, aes(x=Price)) + geom_boxplot(color=	'#275136')
```
...
The correlation of Review Scores with Price is not very high but they still could be informative for the model.
...
All these characteristics except Listing.Description.Length are tightly associated with the price.

```{r}
df_short |> select(-c(Listing.Description.Length)) -> df_short
```
...
All these features could be important.
...
Except for Breakfast, Washer, Family.kid.friendly, Dryer, Hair.dryer and Iron these amenites could be important.
...
The R-squared is not great with all these features. Let's try to take some irrelevant of highly correlated features out of the equation.

After trying several combinations, it seems that the model doesn't perform better with less features. The original model will be used.
...
The RMSE and Rsquared are similar for train and test, meaning that the model is not under- or overfitting.

## Adjusting the model

However, the performance is far from optimal, so let's check the residual errors.
...
The model fails most with Price range outliers. Probably, by better managing the outliers, we could obtain a better result.
...
```{r}
iqr = quantile(df_short$Price, 0.75) - quantile(df_short$Price, 0.25)
upper_limit = quantile(df_short$Price, 0.75) + iqr*1.5
print(upper_limit)
...
```{r}
df_short |>
  mutate(Price=ifelse(Price>150, 150, Price)) -> df_new
```

```{r}
ggplot(df_new, aes(x=Price)) + geom_boxplot(color=	'#275136')
```
...
```{r}
set.seed(7)
itrain <- sample(1:nrow(df_new),round(nrow(df_new)*0.7))

df.train <- df_new[itrain,]
df.test <- df_new[-itrain,]
...
```{r}
new_model <- lm(data=df.train, formula=Price~ .)
...
The RMSE and Rsquared are better than before trimming the upper outliers. Both values are similar for train and test, meaning that the model is not under- or overfitting.
